...
r=requests.post('http://gemini.igenomecloud.com/webroot_for_python.php')
# base_path = r.text+'/'
base_path=r.text
Project_Dir = os.path.dirname(os.path.dirname(__file__))
# Project_Dir1 = os.path.dirname(os.path.abspath(__file__))
def label_result(data):
    if data["性别"] == [] and data["年龄"] == [] and data['病理诊断']==[] and data['批次日期']==[]:
        rowlist = fromsql()
        ...
        #筛选批次日期，并筛选其他值的情况
        if data['批次日期']:
            # batchdatelist=getEveryDay(data['批次日期'][0], data['批次日期'][1])
            begin_date=data['批次日期'][0]
            end_date=data['批次日期'][1]
...
def batch_part_display(rowlist,sexvalue,agevalue,diagnosisvalue):
    ...
    if (sexvalue==['男','女'] or sexvalue==[]) and (agevalue==[] or agevalue==["0","125"]) and (diagnosisvalue==[] or diagnosisvalue ==['非小细胞肺癌','小细胞肺癌','非鳞性非小细胞肺癌','肺鳞癌','肺腺癌','非腺鳞癌','大细胞癌']):
        ...
        if (pathlist):
            ...
            #
            # 箱线图
            TCGA11_path=Project_Dir+"/config/TCGA11.txt"
            with open(TCGA11_path, "r") as TCGA:
            # with open(r"/disk/linda/IgenomeCloudGemini/WordReport/config/TCGA11.txt", "r") as TCGA:
            # with open(r"/disk/lulu/TCGA11.txt", "r") as TCGA:
                ...
                for item in cancer:
                    ...
                    dict1 = {item: []}
                    num = []
                    TCGA_path=Project_Dir + "/config/TCGA11.txt"
                    with open(TCGA_path, "r") as TCGA1:
                    # with open(r"/disk/linda/IgenomeCloudGemini/WordReport/config/TCGA11.txt", "r") as TCGA1:
            ...
            for key, item in enumerate(pathlist):
                filepath = base_path + item + "T1.Somatic.WES.snv.simp.nor"
            ...
            realprovince = []
            for i in hospital:
                hospital_path=Project_Dir + "/config/hospital.csv"
                # csvFile1 = codecs.open("/disk/lulu/hospital.csv", "r", 'gbk')
                # csvFile1 = codecs.open("/disk/linda/IgenomeCloudGemini/WordReport/config/hospital.csv", "r", 'gbk')
                csvFile1 = codecs.open(hospital_path, "r", 'gbk')
            ...
            allprovince = ["北京", "天津", "上海", "重庆", "河北", "山西", "辽宁", "吉林", "黑龙江", "江苏", "浙江", "安徽", "福建", "江西", "山东",
                           "河南", "湖北", "湖南", "广东", "海南", "四川", "贵州", "云南", "陕西", "甘肃", "青海", "台湾", "内蒙", "广西", "西藏",
                           "宁夏", "新疆", "香港", "澳门"]
            tempprovincelist = []
            # for item in provincelist:
            for item in allprovince:
...
def all_display(rowlist):
    ...
    if (pathlist):
        ...
        allprovince = ["北京","天津","上海","重庆","河北","山西","辽宁","吉林","黑龙江","江苏","浙江","安徽","福建","江西","山东","河南","湖北","湖南","广东","海南","四川","贵州","云南","陕西","甘肃","青海","台湾","内蒙","广西","西藏","宁夏","新疆","香港","澳门"]
        # print (type(set(allprovince)))
        tempprovincelist=[]
        # for item in provincelist:
        for item in allprovince:
            # print(item + "\t" + str(countprovince.count(item)))
...
#     data = {"性别": [], "年龄": [], "病理诊断": [], "批次日期": []}
# #     # data = {"性别": ["男"], "年龄": ["65"], "病理诊断": [], "批次日期": ["2018-01-01","2018-01-10"]}
# #     # data= {"性别": [], "年龄": [], "病理诊断": [], "批次日期": []}
# #     # data = {"性别": [], "年龄": [], "病理诊断": [], "批次日期": ["2018-01-01", "2018-01-10"]}
#     label_result(data)