...
def count_mq(inputItem):
    ...
    for read in bamfile.fetch(inputItem[1], int(inputItem[2]) - dis, int(inputItem[2])):
        # listcov = item.strip().split("\t")
        if read.reference_start >= (int(inputItem[2])-dis) and read.reference_end <= int(inputItem[2]):
            L_count = L_count + 1
            L_mq = L_mq + read.mapping_quality


    for read in bamfile.fetch(inputItem[1], int(inputItem[3]), int(inputItem[3]) + dis):
        # listcov = item.strip().split("\t")
        if read.reference_start >= int(inputItem[3]) and read.reference_end<= (int(inputItem[3])+dis):
            R_count = R_count + 1
