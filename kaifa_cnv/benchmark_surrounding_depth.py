...
def count_cov(inputItem):
    ...
    for item in tb.fetch(inputItem[0], int(inputItem[1]), int(inputItem[2])):
    ...
    for item in tb.fetch(inputItem[0], int(inputItem[1]) - dis, int(inputItem[1])):
        listcov = item.strip().split("\t")
        if int(listcov[2]) <= int(inputItem[1]):
            L_count = L_count + 1
            L_depth = L_depth + float(listcov[3])


    for item in tb.fetch(inputItem[0], int(inputItem[2]), int(inputItem[2]) + dis):
        listcov = item.strip().split("\t")
        if int(listcov[2]) <= int(inputItem[2]) + dis:
            R_count = R_count + 1
            R_depth = R_depth + float(listcov[3])
...
def usage():
    print ('''
      ...
      Options include:
        -i(--inputCBS=) input benchmark file               :inputCBSfile 
