...
def count_cov(inputItem):
    ...
    for item in tb.fetch(inputItem[1], int(inputItem[2]), int(inputItem[3])):
        ...
        if int(listcov[2]) <= int(inputItem[3]):
            C_count = C_count + 1
            C_depth = C_depth + float(listcov[3])

    for item in tb.fetch(inputItem[1], int(inputItem[2]) - dis, int(inputItem[2])):
        listcov = item.strip().split("\t")
    ...
    for item in tb.fetch(inputItem[1], int(inputItem[3]), int(inputItem[3]) + dis):
        listcov = item.strip().split("\t")
        if int(listcov[2]) <= int(inputItem[3]) + dis:
            R_count = R_count + 1
            R_depth = R_depth + float(listcov[3])
