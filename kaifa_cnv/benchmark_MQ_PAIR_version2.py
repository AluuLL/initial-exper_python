...
import pysam
#mapping_quality和paired reads support,并且加入了pair reads 的ratio作为以后的筛选条件
...
def count_mq(inputItem):
    ...
    if int(inputItem[1])-int(distance) >=0:
        for read in bamfile.fetch(inputItem[0], int(inputItem[1]) - int(distance), int(inputItem[1])):
            # listcov = item.strip().split("\t")
            if read.reference_start >= (int(inputItem[1])-int(distance)) and read.reference_end <= int(inputItem[1]):
                L_count = L_count + 1
                L_mq = L_mq + read.mapping_quality
    for read in bamfile.fetch(inputItem[0], int(inputItem[2]), int(inputItem[2]) + int(distance)):
        # listcov = item.strip().split("\t")
        if read.reference_start >= int(inputItem[2]) and read.reference_end<= (int(inputItem[2])+int(distance)):
            R_count = R_count + 1
            R_mq = R_mq + read.mapping_quality
    if (int(inputItem[1])-int(longth) >=0) and  (int(inputItem[2])-int(longth)) >=0:
