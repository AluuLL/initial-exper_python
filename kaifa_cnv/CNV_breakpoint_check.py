...
def search_L_R_break(inputItem):
    ...
    # listprint1 = []  # 最后一个目标扎堆列表
    # listprintrest = [i for i in list if i not in listprint]
    # for i in listprintrest:
    #     if i < int(inputItem[3]) and i > value :  # 判断pair reads的位置是否在终止位置之前且，在刚才的listprit的后面，若是进入该条件，若不是，执行else；
    #         if listprint1:
    #             # num = num + 1
    #             if flag == 0:
    #                 if i - listprint1[0] <= 1000:
    #                     listprint1.append(i)
    #                 else:
    #                     flag = 1
    #                     listlinshi = []
    ...
    #                 else:
    #                     flag = 0
    #                     if len(listprint1) < len(listlinshi):
    #                         listprint1 = listlinshi
    #                     else:
    #                         listlinshi = []
    #         else:
    #             listprint1.append(i)
    ...
    #     value1 = value1 + i
    # if len(listprint1)>0:
    #     inputItem[3] = str(value1 / len(listprint1))
    # count_noise = len(listnoise)
    # inputItem.append(str(count_noise))
    return inputItem
