...
class allCallerSimple():
    ...
    def checkPoint(self,TMBFileName, NeoStatisFileName, MMRFileName, POLFileName):
        ...
        if nonsys_num >= 243:
            ...
            tmb_status = "TMB-H(>243个非同义突变位点)"
        elif nonsys_num < 100:
            nonsys_mean = '低'
            tmb_des = "本检测得到的TMB结果低于平均值"
            tmb_status = "TMB-L(小于100个非同义突变位点)"
        else:
            nonsys_mean = '中'
            tmb_des = "本检测得到的TMB结果位于中位数附近"
            tmb_status = "TMB-M(100-243个非同义突变位点)"
        #
        if sys_num >= 98:
            sys_mean = '高'
        elif sys_num < 40:
    ...
    def medication(self):
        ...
        if (realMSI == 'MSI-H'and mmr_mean == "异常")or (realMSI =='MSI-H' and mmr_mean =='正常') or ((realMSI =='MSI-S'or realMSI =='MSI-L') and mmr_mean == '异常') and tmb_mean == '高':
            Nivolumab_mean = RichText('• 临床试验表明PD-L1表达（TPS>=5%）的非小细胞肺癌晚期患者接受Nivolumab治疗，并未获得比化疗更长的无进展生存期和整体生存期（Checkmate 026）。\n',
                style="HLAMeanStyle")
            Nivolumab_mean.add('• 最新临床试验后的探索性研究数据显示，TMB高的晚期非小细胞肺癌患者接受Nivolumab治疗比化疗可获得更长的无进展生存期（Checkmate 026回溯性研究）。\n',
                style="HLAMeanStyle")
            ...
            Atezolizumab_mean.add(
                ...
                style="HLAMeanStyle")
            Atezolizumab_mean.add('• NCCN指南推荐铂类化药治疗无效或进展后的转移性非小细胞肺癌患者（尤其是检测到PD-L1表达和肿瘤浸润淋巴细胞的患者），可以在二线治疗中使用Atezolizumab（POPLAR）。\n',
                style="HLAMeanStyle")
            Atezolizumab_mean.add('• 临床研究表明TMB水平与Atezolizumab临床治疗效果呈正相关。（10.1016/j.jtho.2016.11.343）。',
                                  style="HLAMeanStyle")
            Durvalumab_mean = RichText('• NCCN指南推荐Durvalumab可作为不可切除的非小细胞肺癌III期患者在同步放化疗后使用的辅助治疗药物（PACIFIC）。\n',
                                       style="HLAMeanStyle")
            Durvalumab_mean.add(
                '• 临床试验表明PD-L1表达高（TPS>=25%）的晚期非小细胞肺癌患者在接受Durvalumab治疗时，比PD-L1表达低或不表达的患者，存在明显的响应率和生存期获益（NCT01693562）。\n',
                style="HLAMeanStyle")
            Durvalumab_mean.add(
                '• 临床试验表明使用Durvalumab对未检测到EGFR/ALK突变，且接受过二线以上系统性治疗后失败的晚期非小细胞肺癌患者的治疗中，PD-L1阳性患者比PD-L1阴性患者具有更高的响应率（ATLANTIC）。\n',
                style="HLAMeanStyle")
            Durvalumab_mean.add(
                '• 非小细胞肺癌患者中正在开展的3个关于Durvalumab药物的三期临床试验包括：ARTIC，MYSTIC，NEPTUNE。',
                style="HLAMeanStyle")
            ...
            medicine_note.add(
                ...
                style="HLAMeanStyle")
            medicine_note.add(
                '•\tCheckmate 026 Phase III：PD-L1表达（TPS>=5%）的晚期NSCLC患者，接受Nivolumab治疗与化疗相比，并未取得预期的临床获益（Median PFS: 4.2 vs. 5.9; Median OS: 14.4 vs. 13.2 ）。\n',
                style="HLAMeanStyle")
            medicine_note.add(
                '•\tCheckmate 026回溯性研究Phase III：TMB高的NSCLC患者群体可在Nivolumab治疗中获益（Median PFS: 9.7 vs. 5.8），TMB低的NSCLC群体在Nivolumab治疗中比化疗具有更短的无进展生存期（Median PFS: 4.1 vs. 6.9）。\n',
                style="HLAMeanStyle")
            medicine_note.add(
            ...
            medicine_note.add(
                ...
                style="HLAMeanStyle")
            medicine_note.add(
                '•\tPOPLAR Phase II：在使用铂药基础的化疗治疗后进展的NSCLC患者中，接受Atezolizumab治疗比接受Docetaxel治疗在总体生存期上有30%的延长；根据2018第二版NCCN非小细胞肺癌专家指南，使用Nivolumab和Atezolizumab无需进行PD-L1免疫组化检测，但根据POPLAR II期临床试验的结果，检测到PD-L1表达和肿瘤浸润淋巴细胞的患者（TC3或IC3）总体的生存期或更高。\n',
                style="HLAMeanStyle")
            medicine_note.add(
        ...
        if (realMSI == 'MSI-H'and mmr_mean == '异常') or (realMSI =='MSI-H' and mmr_mean == '正常') or ((realMSI =='MSI-S' or realMSI =='MSI-L') and mmr_mean == '异常') and (tmb_mean == '低' or tmb_mean == '中'):
            Nivolumab_mean = RichText(
                ...
                style="HLAMeanStyle")
            Nivolumab_mean.add(
                '• 最新临床试验后的探索性研究数据显示，TMB低的晚期非小细胞肺癌肺癌患者接受Nivolumab治疗的中位无进展生存期比化疗更短（Checkmate 026回溯性研究）\n',
                style="HLAMeanStyle")
            Nivolumab_mean.add('• FDA批准转移性非小细胞患者在铂类化药治疗无效或进展后，在二线治疗中使用Nivolumab（Checkmate 057，Checkmate 017）。',
                               style="HLAMeanStyle")
            Pembrolizumab_mean = RichText('• 如果PD-L1免疫组化检测呈阳性（>=50%），可以考虑在一线治疗中使用Pembrolizumab（Keynote 024）。\n',
            ...
            Atezolizumab_mean.add(
                ...
                style="HLAMeanStyle")
            Atezolizumab_mean.add(
                '• NCCN指南推荐铂类化药治疗无效或进展后的转移性非小细胞肺癌患者（尤其是检测到PD-L1表达和肿瘤浸润淋巴细胞的患者），可以在二线治疗中使用Atezolizumab（POPLAR）。',
                style="HLAMeanStyle")
            Durvalumab_mean = RichText('• NCCN指南推荐Durvalumab可作为不可切除的非小细胞肺癌III期患者在同步放化疗后使用的辅助治疗药物（PACIFIC）。\n',
                                       style="HLAMeanStyle")
            Durvalumab_mean.add(
            ...
            medicine_note.add(
                ...
                style="HLAMeanStyle")
            medicine_note.add(
                '•\tCheckmate 026 Phase III回溯性研究：TMB高的NSCLC患者群体可在Nivolumab治疗中获益（Median PFS: 9.7 vs. 5.8），TMB低的NSCLC群体在Nivolumab治疗中比化疗具有更短的无进展生存期（Median PFS: 4.1 vs. 6.9）。\n',
                style="HLAMeanStyle")
            medicine_note.add(
                '•\tCheckmate 057 Phase III：转移性非鳞性NSCLC患者接受Nivolumab单药治疗比接受Docetaxel治疗的总体生存期更长（24-month OS：29% vs. 16% ）。\n',
                style="HLAMeanStyle")
        ...
        if ((realMSI == 'MSI-S' or realMSI == 'MSI-L')and mmr_mean =='正常') and tmb_mean =='高' :
            Nivolumab_mean = RichText(
                ...
                style="HLAMeanStyle")
            Nivolumab_mean.add(
                '• 最新临床试验后的探索性研究数据显示，TMB高的晚期非小细胞肺癌患者接受Nivolumab治疗比化疗可获得更长的无进展生存期（Checkmate 026回溯性研究）。\n',
                style="HLAMeanStyle")
            Nivolumab_mean.add('• FDA批准转移性非小细胞患者在铂类化药治疗无效或进展后，在二线治疗中使用Nivolumab（Checkmate 057，Checkmate 017）。\n',
                               style="HLAMeanStyle")
            Nivolumab_mean.add(
