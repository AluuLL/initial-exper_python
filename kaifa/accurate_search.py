...
def fromsql(content):
    ...
    sql = "select s.batchid, s.basic_json, s.sample_data, t.outputfile from ge_task t inner join ge_sample s on t.sample_uniqueid = s.sample_uniqueid WHERE s.sample_data = 1 and s.batchid in ( % s) " % (so_content)
    try:
        cursor.execute(sql)
